...
bool app_timer_is_stopped = false;
...
struct Light {
    ...
    void setupCameraForShadowMapping(glm::vec3 scene_center, float scene_radius) {
        glm::mat4 lightP = glm::ortho(-scene_radius, scene_radius, -scene_radius, scene_radius, 0.1f, scene_radius * 5.0f);
        glm::mat4 lightV = glm::lookAt(m_position, scene_center, glm::vec3(0.0f, 1.0f, 0.0f));
        depthMVP = lightP * lightV;
        m_direction = glm::normalize(scene_center - m_position);
    }
...
struct Scene {
    ...
    std::shared_ptr<ShaderProgram> shadowMapShaderProgramPtr;
    ...
    void render() {
        ...
        */
        ...
        for (int i = 0; i < scene_lights.size(); ++i) {
            ...
            shadowMapShaderProgramPtr->set("depthMVP", light.depthMVP);
            ...
            if (save_shadow_maps_in_ppm) {
                light.shadowMap->save_PPM_file(std::string("shadow_map_") + std::to_string(i) + std::string(".ppm"));
            }
        }
        shadowMapShaderProgramPtr->stop();
        ...
        // Set the lights in the shader :
        {
            for (int i = 0; i < scene_lights.size(); ++i) {
                ...
                shaderProgramPtr->set(std::string("lightSources[" + std::to_string(i) + "].direction"), light.m_direction);
                shaderProgramPtr->set(std::string("lightSources[" + std::to_string(i) + "].isActive"), 1);
                shaderProgramPtr->set(std::string("lightSources[" + std::to_string(i) + "].depthMVP"), light.depthMVP); //Pass transform light
            }
        }
...
void initOpenGL() {
    ...
    try {
        scene.shadowMapShaderProgramPtr = ShaderProgram::genBasicShaderProgram(SHADER_PATH + "VertexShader_shadowMap.glsl",
            SHADER_PATH + "FragmentShader_shadowMap.glsl");
        scene.shadowMapShaderProgramPtr->stop();
...
void initScene(const std::string& meshFilename) {
    ...
    // Setup lights :
    {
        unsigned int shadow_map_width = 1024, shadow_map_height = 1024; // play with these parameters
        std::vector<glm::vec3> colors = {glm::vec3(1.0, 0.0, 0.0), glm::vec3(0.0, 1.0, 0.0), glm::vec3(0.0, 0.0, 1.0), glm::vec3(1.0, 1.0, 1.0)};
        int nb_lights = 4;
        {
            ...
            for (int i = 0; i < nb_lights; ++i) {
                ...
                newLight.m_position = glm::vec3(cos(app_timer + i * 2 * pi / nb_lights), sin(app_timer + i * 2 * pi / nb_lights), 4.0);
                newLight.m_color = colors[i % colors.size()];
                newLight.m_intensity = 1.0f;
                newLight.allocateShadowMapFBO(shadow_map_width, shadow_map_height);
                newLight.shadowMapTexOnGPU = texture_slot_available++;
            }
        }
...
void clear() {
    ...
    scene.shadowMapShaderProgramPtr.reset();
...
void update(float currentTime) {
    if (!app_timer_is_stopped) {
        ...
        for (int i = 0; i < scene.scene_lights.size(); ++i) {
            Light& light = scene.scene_lights[i];
            light.m_position = glm::vec3(cos(app_timer + i * 2 * pi / scene.scene_lights.size()), sin(app_timer + i * 2 * pi / scene.scene_lights.size()), 4.0);
        }
    }
}
